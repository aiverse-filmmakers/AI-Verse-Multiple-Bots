...
export class TeamRunFanout {
  ...
  createFanout(input: CreateFanoutInput): FanoutSnapshot {
    ...
    if (permissions.can_create_workers !== true) throw new Error(`Bot ${leaderId} is not explicitly allowed to create temporary Workers`);

    this.assertNoActiveFanout(run);
    this.assertNoLiveWorkerTasks(run.id);

    const runBudget = normalizeBudget(run.payload.budget);
    const activeWorkerStates = new Set(["created", "ready", "running", "waiting"]);
    const existingWorkers = this.teams.listWorkers(run.id).filter((worker) => activeWorkerStates.has(String(worker.payload.status)));
    if (typeof runBudget.max_workers === "number" && existingWorkers.length + input.workers.length > runBudget.max_workers) {
      throw new BudgetError(
        ...
        `Team Run ${run.id} would have ${existingWorkers.length + input.workers.length} active Workers with a limit of ${runBudget.max_workers}`
      );
    }
    ...
    const runPayload = validateProtocolObject({
      ...
      worker_ids: [...new Set([...stringArray(run.payload.worker_ids), ...planned.map((item) => item.workerId)])],
      task_ids: [...new Set([...stringArray(run.payload.task_ids), ...planned.map((item) => item.taskId)])],
    ...
    const creation = this.gateway.store.atomicMutation({
      preconditions: [
      ...
      objects: [
        ...
        ...planned.flatMap((item) => [
          ...
          { kind: "environment_lease" as const, payload: item.environmentLease },
    ...
    this.gateway.events.publishCommitted(creation.events);
  ...
  private activatePreparedFanout(runId: string, fanoutId: string, recovered: boolean): void {
    for (let attempt = 0; attempt < OPTIMISTIC_RETRY_LIMIT; attempt += 1) {
      ...
      const objects = [
        ...
        ...tasks.map((task) => ({ kind: "task" as const, payload: validateProtocolObject({ ...task.payload, status: "assigned", execution_state: "queued", assigned_at: timestamp }, "task") })),
  ...
  private async reconcileRunUnlocked(runId: string, requestedFanoutId?: string): Promise<void> {
    ...
    this.syncWorkerLifecycle(runId, fanoutId);
  ...
  private syncWorkerLifecycle(runId: string, fanoutId: string): void {
    const snapshot = this.snapshot(runId, fanoutId);
    const leaderId = String(snapshot.run.payload.leader_id ?? "system_fanout");
    const taskById = new Map(snapshot.tasks.map((task) => [task.id, task]));

    for (const worker of snapshot.workers) {
      const task = taskById.get(String(worker.payload.task_id ?? ""));
      if (!task) throw new Error(`Fan-out Worker ${worker.id} has no canonical Task`);
      ...
      if (new Set(["completed", "failed", "canceled", "expired"]).has(current)) {
        throw new Error(`Fan-out Worker ${worker.id} is terminal as ${current} while Task ${task.id} is ${taskStatus}`);
      }
      ...
      const payload = validateProtocolObject({
        ...
        ...(target === "ready" && current === "running" ? { recovery_reconciled_at: timestamp } : {})
      }, "worker");
      const event = this.event({
        type: `worker.${target}`,
        actorId: leaderId,
        workspaceId: String(snapshot.run.payload.workspace_id),
        runId,
        taskId: task.id,
        correlationId: String(snapshot.run.payload.root_objective_id),
        summary: `Reconciled fan-out Worker ${worker.id} to ${target} from canonical Task ${task.id}`
      });
  ...
  private planWorker(run: StoredObject, leader: StoredObject, spec: FanoutWorkerInput, budget: BudgetEnvelope, fanoutId: string): PlannedWorker {
    ...
    const parentTaskId = spec.parentTaskId
      ?? (typeof run.payload.parent_task_id === "string" && run.payload.parent_task_id.length > 0 ? run.payload.parent_task_id : undefined);
    ...
    const leaseExpiresAt = this.resolveLeaseExpiry(spec.leaseExpiresAt);
    const runtime = this.resolveWorkerRuntime(leader, spec);
    const execution = this.resolveWorkerExecution(leader, String(run.payload.workspace_id), spec);
    const timestamp = nowIso();
    ...
    const lease = validateProtocolObject({
      ...
      connections: [...new Set(spec.connections ?? [])],
    ...
    const worker = validateProtocolObject({
      ...
      environment_lease_id: environmentLeaseId,
      budget: prepared.budget,
      output_contract: spec.expectedOutput ?? { contract: "artifact-or-structured-result" },
      required_constraints: requiredConstraints,
      status: "created",
      created_at: timestamp,
      updated_at: timestamp
    }, "worker");
    return { workerId, taskId, leaseId, environmentLeaseId, worker, task, lease, environmentLease, recoveryPolicy, maxAttempts };
  }

  private resolveLeaseExpiry(value?: string): string {
    const expiresAt = value ?? new Date(Date.now() + 60 * 60 * 1000).toISOString();
    const timestamp = Date.parse(expiresAt);
    if (!Number.isFinite(timestamp) || timestamp <= Date.now()) throw new Error("Fan-out Worker lease expiry must be a future timestamp");
    return new Date(timestamp).toISOString();
  }

  private resolveWorkerRuntime(leader: StoredObject, input: FanoutWorkerInput): JsonObject {
    const leaderRuntime = asObject(leader.payload.runtime);
    for (const key of ["api_key", "token", "authorization"]) {
      if (typeof leaderRuntime[key] === "string") throw new Error(`Raw runtime credential ${key} is forbidden when creating temporary Workers`);
    }
  ...
  private resolveWorkerExecution(leader: StoredObject, workspaceId: string, input: FanoutWorkerInput): JsonObject {
    ...
    if (environmentPolicy === "shared_workspace") {
      return { environment_policy: "shared_workspace", environment_ref: `workspace:${workspaceId}`, persistence: "run_scoped" };
    }
    const environmentRef = input.environmentRef?.trim();
    if (!environmentRef) throw new Error(`${environmentPolicy} Worker execution requires a trusted environmentRef from runtime infrastructure`);
    return { environment_policy: environmentPolicy, environment_ref: environmentRef, persistence: "run_scoped" };
  }
  ...
  private ensureRunning(runId: string, actorId: string): StoredObject {
    ...
    if (status === "created") {
      run = this.teams.transitionRun(runId, "planning", actorId, "Parallel fan-out planning started").object;
      run = this.teams.transitionRun(runId, "running", actorId, "Parallel fan-out execution started").object;
    } else if (status === "planning" || status === "waiting_input" || status === "waiting_approval") {
      run = this.teams.transitionRun(runId, "running", actorId, "Parallel fan-out execution resumed").object;
