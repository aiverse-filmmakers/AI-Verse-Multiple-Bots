...
for row in rows:
    items.append({
        ...
        "why": get(row, "why"),
        "path": get(row, "path"),
        "source_identity": get(row, "source_identity"),
        "source_version": get(row, "source_version"),
        "freshness": get(row, "freshness"),
...
export interface AiVerseMemoryCompatibility {
  status: MemoryRecallCompatibilityStatus;
  root: string;
  entrypoint: string;
...
export interface MemoryRecallDirective {
  query: string;
  limit: number;
...
export interface MemoryRecallProvider {
  recall(request: MemoryRecallRequest): Promise<MemoryRecallProjection>;
}
...
export type MemoryCommandRunner = (
  executable: string,
  args: string[],
...
export interface AiVerseMemoryRecallProviderOptions {
  ...
  timeoutMs?: number;
  commandRunner?: MemoryCommandRunner;
}
...
function requiredString(value: unknown, label: string, maxLength = 2_048): string {
  if (typeof value !== "string" || value.trim().length === 0) {
    throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_INVALID", `${label} must be a non-empty string`);
  }
  const normalized = value.trim();
  if (normalized.length > maxLength) {
    throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_TOO_LARGE", `${label} exceeds ${maxLength} characters`);
  }
  return normalized;
}
...
function optionalFinite(value: unknown): number | null {
  ...
  if (!Number.isFinite(numberValue)) throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_INVALID", "Memory response numeric field is invalid");
  return numberValue;
}
...
function assertWorkspaceId(workspaceId: string): string {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(value)) {
    throw new AiVerseMemoryRecallError("MEMORY_WORKSPACE_INVALID", `Invalid AI-Verse workspace id ${workspaceId}`);
  }
  return value;
}
...
function safeRelativePath(value: unknown, workspaceId: string, scope: string): string {
  ...
  if (!path.startsWith(expectedPrefix)) {
    throw new AiVerseMemoryRecallError("MEMORY_SCOPE_VIOLATION", `Memory source ${path} is outside authorized scope ${scope}`);
  }
  return path;
}
...
function parseEngineVersion(enginePath: string): string | null {
  try {
    ...
    const match = text.match(/(?m)^VERSION\s*=\s*["']([^"']+)["']/);
    return match?.[1] ?? null;
  } catch {
    return null;
  }
...
export function detectAiVerseMemoryCompatibility(osRoot: string): AiVerseMemoryCompatibility {
  ...
  const present = [entrypoint, engine, osCompat].filter((path) => existsSync(path));
  if (present.length === 0) {
    return {
      status: "absent",
      root,
  ...
  if (os.status !== "compatible") {
    return {
      status: "invalid",
      root,
      entrypoint,
      providerVersion: null,
      reason: `AI-Verse Memory native recall requires a compatible AI-Verse OS v2 host: ${os.reason}`
    };
  }

  const missing = [entrypoint, engine, osCompat].filter((path) => !existsSync(path));
  if (missing.length > 0) {
    return {
      status: "invalid",
      root,
      entrypoint,
      providerVersion: null,
      reason: `AI-Verse Memory installation is incomplete: missing ${missing.map((path) => path.slice(root.length + 1)).join(", ")}`
    };
  }

  const unsafe = [entrypoint, engine, osCompat].find((path) => !regularNonSymlink(path));
  if (unsafe) {
    return {
      status: "invalid",
      root,
      entrypoint,
      providerVersion: null,
      reason: `AI-Verse Memory component must be a regular non-symlink file: ${unsafe.slice(root.length + 1)}`
    };
  }
  ...
  if (!providerVersion) {
    return {
      ...
      entrypoint,
      providerVersion: null,
      reason: "AI-Verse Memory engine does not expose a readable VERSION contract"
    };
  }
...
export function normalizeMemoryRecallDirective(value: unknown): MemoryRecallDirective | null {
  ...
  const unknown = Object.keys(source).filter((key) => !new Set(["query", "limit", "include_history"]).has(key));
  if (unknown.length > 0) {
    throw new AiVerseMemoryRecallError("MEMORY_RECALL_DIRECTIVE_INVALID", `Unsupported memory_recall field(s): ${unknown.join(", ")}`);
  }

  if (typeof source.query !== "string" || source.query.trim().length === 0) {
    throw new AiVerseMemoryRecallError("MEMORY_RECALL_QUERY_REQUIRED", "memory_recall.query must be a non-empty string");
  }
  const query = source.query.trim();
  if (query.length > MAX_QUERY_CHARS || query.includes("\0")) {
    throw new AiVerseMemoryRecallError("MEMORY_RECALL_QUERY_INVALID", `memory_recall.query must be at most ${MAX_QUERY_CHARS} characters and contain no NUL bytes`);
  }
  ...
  if (typeof rawLimit !== "number" || !Number.isInteger(rawLimit) || rawLimit < 1 || rawLimit > MAX_LIMIT) {
    throw new AiVerseMemoryRecallError("MEMORY_RECALL_LIMIT_INVALID", `memory_recall.limit must be an integer from 1 to ${MAX_LIMIT}`);
  }
  if (source.include_history !== undefined && typeof source.include_history !== "boolean") {
    throw new AiVerseMemoryRecallError("MEMORY_RECALL_HISTORY_INVALID", "memory_recall.include_history must be boolean when provided");
  }

  return {
    query,
    limit: rawLimit,
    include_history: source.include_history === true
  };
}
...
async function defaultCommandRunner(
  ...
  if (!childProcess?.execFile) {
    throw new AiVerseMemoryRecallError("MEMORY_PROCESS_UNAVAILABLE", "Node child_process.execFile is unavailable");
  }
    ...
    childProcess.execFile(
      ...
      (error: any, stdout: unknown, stderr: unknown) => {
        if (error) {
          ...
          (wrapped as any).killed = error.killed;
          (wrapped as any).signal = error.signal;
          (wrapped as any).stderr = String(stderr ?? "");
...
export class AiVerseMemoryRecallProvider implements MemoryRecallProvider {
  ...
  constructor(readonly osRoot: string, options: AiVerseMemoryRecallProviderOptions = {}) {
    ...
    if (this.compatibility.status !== "available") {
      throw new AiVerseMemoryRecallError(
        this.compatibility.status === "absent" ? "MEMORY_NOT_INSTALLED" : "MEMORY_INSTALL_INVALID",
        this.compatibility.reason
      );
    }
  ...
  async recall(request: MemoryRecallRequest): Promise<MemoryRecallProjection> {
    ...
    const directive = normalizeMemoryRecallDirective({
      query: request.query,
    ...
    const raw = await this.invokeBridge(workspaceId, directive);
    const response = asRecord(raw);
    if (response.provider !== "ai-verse-memory") {
      throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_INVALID", "Memory bridge returned an unexpected provider");
    }
    if (response.mode !== "ai-verse-os-v2") {
      throw new AiVerseMemoryRecallError("MEMORY_MODE_INVALID", `Memory bridge returned unsupported mode ${String(response.mode)}`);
    }
    if (response.workspace_id !== workspaceId) {
      throw new AiVerseMemoryRecallError("MEMORY_SCOPE_VIOLATION", `Memory bridge returned workspace ${String(response.workspace_id)} for ${workspaceId}`);
    }
    ...
    if (rawItems.length > directive.limit || rawItems.length > MAX_LIMIT) {
      throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_TOO_LARGE", `Memory bridge returned ${rawItems.length} items for limit ${directive.limit}`);
    }
    ...
    for (const rawItem of rawItems) {
      ...
      if (!ALLOWED_KINDS.has(kind)) {
        throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_INVALID", `Memory item ${id} has unsupported kind ${kind}`);
      }
      ...
      if (scope !== "operator" && scope !== `workspace:${workspaceId}`) {
        throw new AiVerseMemoryRecallError("MEMORY_SCOPE_VIOLATION", `Memory item ${id} escaped workspace ${workspaceId}: ${scope}`);
      }
      const status = requiredString(source.status ?? "active", `Memory item ${id} status`, 64);
      if (!directive.include_history && status !== "active") {
        throw new AiVerseMemoryRecallError("MEMORY_HISTORY_VIOLATION", `Memory item ${id} returned non-active status without include_history`);
      }
      const text = typeof source.text === "string" ? source.text.trim() : "";
      const why = typeof source.why === "string" ? source.why.trim() : "";
      if (text.length === 0) throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_INVALID", `Memory item ${id} has no text`);
      if (text.length > MAX_ITEM_TEXT_CHARS || why.length > MAX_ITEM_WHY_CHARS) {
        throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_TOO_LARGE", `Memory item ${id} exceeds bounded text limits`);
      }
      totalText += text.length + why.length;
      if (totalText > MAX_TOTAL_TEXT_CHARS) {
        throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_TOO_LARGE", `Memory recall exceeds ${MAX_TOTAL_TEXT_CHARS} total text characters`);
      }
      ...
      if (!SHA256_PATTERN.test(sourceIdentity) || !SHA256_PATTERN.test(sourceVersion)) {
        throw new AiVerseMemoryRecallError("MEMORY_PROVENANCE_INVALID", `Memory item ${id} lacks canonical SHA-256 source provenance`);
      }
      const freshness = source.freshness;
      if (freshness !== "fresh" && freshness !== "historical") {
        throw new AiVerseMemoryRecallError("MEMORY_PROVENANCE_INVALID", `Memory item ${id} has invalid freshness ${String(freshness)}`);
      }
      ...
      const itemBase = {
        ...
        scope,
        status,
        importance: optionalFinite(source.importance),
        confidence: optionalFinite(source.confidence),
        updated_at: optionalString(source.updated_at, 128),
        source: optionalString(source.source, 2_048),
        text,
        why,
        path,
      ...
      items.push({
        ...itemBase,
        item_digest: sha256(JSON.stringify(itemBase))
      });
    }

    const queryDigest = sha256(directive.query);
    const recallDigest = sha256(JSON.stringify({
      workspace_id: workspaceId,
      query_digest: queryDigest,
      include_history: directive.include_history,
    ...
    return {
      ...
      query_digest: queryDigest,
      requested_limit: directive.limit,
      include_history: directive.include_history,
  ...
  private async invokeBridge(workspaceId: string, directive: MemoryRecallDirective): Promise<unknown> {
    const args = [
      ...
      workspaceId,
      directive.query,
      String(directive.limit),
      directive.include_history ? "1" : "0"
    ];
    ...
    for (const executable of this.python) {
      try {
        const result = await this.commandRunner(executable, args, {
          cwd: this.compatibility.root,
          timeoutMs: this.timeoutMs,
          maxBuffer: MAX_PROCESS_OUTPUT_BYTES
        });
        if (result.stderr.trim().length > 0 && result.stdout.trim().length === 0) {
          throw new AiVerseMemoryRecallError("MEMORY_PROCESS_FAILED", `AI-Verse Memory recall failed: ${result.stderr.replace(/\s+/g, " ").slice(0, 500)}`);
        }
        try {
          return JSON.parse(result.stdout);
        } catch {
          throw new AiVerseMemoryRecallError("MEMORY_RESPONSE_INVALID", "AI-Verse Memory bridge returned invalid JSON");
        }
      } catch (error) {
        lastError = error;
        ...
        throw new AiVerseMemoryRecallError(
          (error as any)?.killed ? "MEMORY_PROCESS_TIMEOUT" : "MEMORY_PROCESS_FAILED",
          `AI-Verse Memory recall process failed${stderr ? `: ${stderr}` : `: ${error instanceof Error ? error.message : String(error)}`}`
        );
      }
    }
    throw new AiVerseMemoryRecallError(
...
export function createOptionalAiVerseMemoryRecallProvider(
  osRoot: string,
  options: AiVerseMemoryRecallProviderOptions = {}
): AiVerseMemoryRecallProvider | undefined {
  ...
  if (compatibility.status === "invalid") {
    throw new AiVerseMemoryRecallError("MEMORY_INSTALL_INVALID", compatibility.reason);
  }
