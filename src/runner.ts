...
export class BotRunner {
  ...
  private async cancelTree(task: StoredObject, actorId: string, reason: string, tasks: StoredObject[], events: AppendedEvent[], visited: Set<string>): Promise<void> {
    ...
    let canceled = this.store.putObject("task", validateProtocolObject({ ...current.payload, status: "canceled", canceled_at: nowIso(), canceled_by: actorId, cancellation_reason: reason, cancellation_code: "CANCELED" }, "task"));
    const event = this.gateway.emit({ type: "task.canceled", actorId, workspaceId: current.workspaceId, taskId: current.id, correlationId: String(current.payload.root_objective_id), summary: reason, attentionState: "canceled" });
