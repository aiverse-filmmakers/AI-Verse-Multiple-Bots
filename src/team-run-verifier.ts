...
export type VerificationScheduleResult =
  ...
  | { status: "scheduled"; run: StoredObject; worker: StoredObject; task: StoredObject; capabilityLease: StoredObject; environmentLease: StoredObject; reportRefs: string[] };
...
export class TeamRunVerifier {
  ...
  schedule(input: ScheduleVerificationInput): VerificationScheduleResult {
    ...
    if (!new Set<TeamRunStatus>(["running", "verifying"]).has(runStatus)) {
      throw new Error(`Team Run ${run.id} cannot schedule verifier work from status ${runStatus}`);
    }
    ...
    const requested = input.reportRefs === undefined ? debtRefs : unique(input.reportRefs.map((ref) => ref.trim()).filter(Boolean));
    ...
    const spawned = this.teams.spawnWorker({
      ...
      inputArtifactRefs: sourceArtifactRefs,
      tools: input.tools ?? [],
    ...
    const runPayload = validateProtocolObject({
      ...spawned.run.payload,
      status: "verifying",
      active_verification_task_id: spawned.task.id,
      verification_task_ids: unique([...stringArray(spawned.run.payload.verification_task_ids), spawned.task.id]),
      verification_pending_report_refs: requested,
      verification_started_at: timestamp,
      verification_ready_for_synthesis: false,
      updated_at: timestamp
    }, "team_run");
    ...
    const mutation = this.gateway.store.atomicMutation({
      preconditions: [
        { id: spawned.run.id, kind: "team_run", status: String(spawned.run.payload.status), updatedAt: spawned.run.updatedAt },
        { id: spawned.worker.id, kind: "worker", status: "created" },
        { id: spawned.task.id, kind: "task", status: "created", ownerId: spawned.worker.id }
      ],
      objects: [
      ...
      queueInsert: {
        id: executionId,
        itemKind: "task",
        itemId: spawned.task.id,
        targetId: spawned.worker.id,
        workspaceId: String(spawned.run.payload.workspace_id),
        state: "queued",
        attempts: 0,
        maxAttempts,
        recoveryPolicy,
    ...
    let execution = this.queue.getByItem(spawned.task.id);
    if (!execution) {
      execution = this.queue.enqueueTask(spawned.task.id, spawned.worker.id, String(spawned.run.payload.workspace_id), {
        recoveryPolicy,
        maxAttempts
      });
    }

    const updatedRun = mutation.objects.find((object) => object.id === spawned.run.id);
    const worker = mutation.objects.find((object) => object.id === spawned.worker.id);
    const task = mutation.objects.find((object) => object.id === spawned.task.id);
    if (!updatedRun || !worker || !task || !execution) throw new Error(`Verifier scheduling for ${spawned.run.id} committed incompletely`);
    ...
    return {
      ...
      worker,
      task,
      capabilityLease: spawned.capabilityLease,
      environmentLease: spawned.environmentLease,
      reportRefs: requested
    };
  }
  ...
  reconcileTask(taskId: string): VerificationReconcileResult | null {
    for (let attempt = 0; attempt < OPTIMISTIC_RETRY_LIMIT; attempt += 1) {
      ...
      if (!TERMINAL_TASK_STATES.has(status)) {
        this.syncWorkerLifecycle(task, run);
        return {
          run: this.requireRun(run.id),
          task: this.gateway.store.getObject(task.id) ?? task,
          verdict: null,
          outcome: null,
          resolvedReportRefs: [],
          remainingReportRefs: stringArray(run.payload.verification_required_report_refs)
        };
      }
      ...
      if (existingVerdict) {
        this.syncWorkerLifecycle(task, run);
        return this.resultFromVerdict(this.requireRun(run.id), task, existingVerdict);
      }
      ...
      if (status === "canceled") {
        ...
        failureReason = String(task.payload.cancellation_reason ?? task.payload.cancel_reason ?? task.payload.failure_reason ?? "Verifier Task canceled");
        ...
        try {
          ...
          const parsed = this.parseRuntimeVerdict(rawArtifact, reportRefs, run, task);
      ...
      const readyForSynthesis = remainingReportRefs.length === 0 && status === "completed" && outcome === "resolved";
      const updatedRunPayload = validateProtocolObject({
        ...
        verification_ready_for_synthesis: readyForSynthesis,
        verification_settled_at: timestamp,
        updated_at: timestamp
      }, "team_run");

      const worker = this.requireVerifierWorker(task, run);
      const targetWorkerStatus = status === "completed" ? "completed" : status === "canceled" ? "canceled" : "failed";
      const currentWorkerStatus = String(worker.payload.status);
      if (new Set(["completed", "failed", "canceled", "expired"]).has(currentWorkerStatus) && currentWorkerStatus !== targetWorkerStatus) {
        throw new Error(`Verifier Worker ${worker.id} is terminal as ${currentWorkerStatus} while Task ${task.id} is ${status}`);
      }
      const workerPayload = validateProtocolObject({
        ...worker.payload,
        ...
        updated_at: timestamp,
        ended_at: typeof worker.payload.ended_at === "string" ? worker.payload.ended_at : timestamp
      }, "worker");
      ...
      try {
        ...
        if (!verdict || !updatedRun) throw new Error(`Verifier reconciliation for ${task.id} committed incompletely`);
        this.gateway.emit({
          ...
          attentionState: remainingReportRefs.length > 0 || outcome === "verifier_failed" ? "unread_result" : undefined,
        ...
        if (readyForSynthesis) {
          this.gateway.emit({
            ...
            taskId: task.id,
            correlationId: String(run.payload.root_objective_id),
            summary: `Verification debt is resolved for Team Run ${run.id}`,
            idempotencyKey: `verification:${run.id}:${verdictId}:ready`
          });
        }
  ...
  recoverPendingVerifications(): VerificationReconcileResult[] {
    ...
    for (const run of this.teams.listRuns()) {
      ...
      } else if (task.payload.status === "running") {
        this.syncWorkerLifecycle(task, run);
      }
    }
    return recovered;
  ...
  private parseRuntimeVerdict(rawArtifact: StoredObject, expectedReportRefs: string[], run: StoredObject, task: StoredObject): ParsedReportVerdict[] {
    if (rawArtifact.workspaceId !== run.workspaceId) throw new Error("Verifier runtime Artifact escaped Team Run workspace");
    if (String(rawArtifact.payload.task_id ?? "") !== task.id) throw new Error(`Verifier runtime Artifact ${rawArtifact.id} is not bound to Task ${task.id}`);
    const artifactTask = this.gateway.store.getObject(String(rawArtifact.payload.task_id ?? ""));
    if (!artifactTask || artifactTask.kind !== "task" || String(artifactTask.payload.run_id ?? "") !== run.id || artifactTask.payload.verification_contract !== "verifier-verdict-v1") {
      throw new Error("Verifier runtime Artifact escaped Team Run Task lineage");
    }
  ...
  private assertLeaderAuthority(leader: StoredObject, tools: string[], connections: string[]): void {
    ...
    if (permissions.can_create_workers !== true) throw new Error(`Team Run leader ${leader.id} is not explicitly allowed to create verifier Workers`);
  ...
  private requireVerifierWorker(task: StoredObject, run: StoredObject): StoredObject {
    const workerId = String(task.payload.assignee_id ?? "");
    const worker = this.gateway.store.getObject(workerId);
    if (!worker || worker.kind !== "worker") throw new Error(`Verifier Worker ${workerId} not found`);
    if (worker.workspaceId !== run.workspaceId || String(worker.payload.run_id ?? "") !== run.id || String(worker.payload.task_id ?? "") !== task.id) {
      throw new Error(`Verifier Worker ${worker.id} is outside Task ${task.id} Team Run scope`);
    }
    if (worker.payload.verifier_role !== true) throw new Error(`Worker ${worker.id} is not bound as a verifier`);
    return worker;
  }

  private syncWorkerLifecycle(task: StoredObject, run: StoredObject): void {
    const worker = this.requireVerifierWorker(task, run);
    const taskStatus = String(task.payload.status);
    const target = taskStatus === "assigned"
      ? "ready"
      : taskStatus === "running"
        ? "running"
        : taskStatus === "completed"
          ...
          : taskStatus === "failed"
            ...
            : taskStatus === "canceled"
              ? "canceled"
              : null;
    if (!target || String(worker.payload.status) === target) return;
    const current = String(worker.payload.status);
    if (new Set(["completed", "failed", "canceled", "expired"]).has(current)) {
      throw new Error(`Verifier Worker ${worker.id} is terminal as ${current} while Task ${task.id} is ${taskStatus}`);
    }
    const timestamp = nowIso();
    const payload = validateProtocolObject({
      ...
      status: target,
      updated_at: timestamp,
      ...(target === "running" && typeof worker.payload.started_at !== "string" ? { started_at: timestamp } : {}),
      ...(new Set(["completed", "failed", "canceled"]).has(target) ? { ended_at: timestamp } : {})
    }, "worker");
    this.gateway.store.atomicMutation({
      preconditions: [
        { id: task.id, kind: "task", status: taskStatus, ownerId: String(task.payload.owner_id) },
        { id: worker.id, kind: "worker", status: current }
      ],
      objects: [{ kind: "worker", payload }],
      events: []
    });
  }
