...
function oppositeStance(a: ClaimStance | null, b: ClaimStance | null): boolean {
...
/**
 * Deterministic, host-neutral disagreement analysis over structured candidate
 * Artifacts. This layer compares declared claims only; it never asks for or
 * infers hidden chain-of-thought.
 */
export class TeamRunDisagreementDetector {
  ...
  analyze(input: AnalyzeDisagreementInput): DisagreementAnalysisResult {
    ...
    const requestedMax = input.maxArtifacts ?? DEFAULT_MAX_ARTIFACTS;
    if (!Number.isInteger(requestedMax) || requestedMax < 1 || requestedMax > ABSOLUTE_MAX_ARTIFACTS) {
    ...
    for (let attempt = 0; attempt < OPTIMISTIC_RETRY_LIMIT; attempt += 1) {
      ...
      const effectiveMax = Math.min(requestedMax, typeof runBudget.max_tasks === "number" ? runBudget.max_tasks : requestedMax);
      if (refs.length > effectiveMax) {
        throw new BudgetError(
          "DISAGREEMENT_INPUT_LIMIT",
          `Disagreement analysis received ${refs.length} candidate Artifacts but Team Run ${run.id} allows at most ${effectiveMax} for one bounded comparison`
        );
      }
      ...
      const analysis = this.compare(extraction.claims, refs.length, threshold);
      ...
      if (existing) {
        if (existing.kind !== "artifact" || existing.payload.kind !== "disagreement_report" || existing.payload.run_id !== run.id) {
          throw new Error(`Deterministic disagreement report ID ${reportId} is already bound to another object`);
        }
        this.emitAnalyzed(existing, input.actorId, run);
        return this.result(run, existing);
      }

      if (!ANALYZABLE_RUN_STATES.has(String(run.payload.status))) {
        throw new Error(`Team Run ${run.id} cannot create disagreement analysis from status ${String(run.payload.status)}`);
      }
      ...
      const reportPayload = validateProtocolObject({
        ...
        run_id: run.id,
        task_id: null,
        media_type: "application/vnd.ai-verse.disagreement-report+json",
        ...
        inline_content: {
          ...
          source_artifact_refs: refs,
          analyzed_artifact_count: refs.length,
          analyzed_claim_count: extraction.claims.length,
          comparable_pair_count: analysis.comparablePairCount,
          confidence_gap_threshold: threshold,
          findings: analysis.findings,
          compatible_subjects: analysis.compatibleSubjects,
          invalid_claims: extraction.invalidClaims,
          claim_contract: {
            accepted_container: "inline_content.claims[]",
            required: ["subject|key|topic"],
            supported_kinds: [...CLAIM_KINDS],
            optional: ["value|position", "stance", "confidence", "evidence_refs", "exclusive_group", "relation", "tolerance"]
          }
        },
      ...
      try {
        ...
        this.emitAnalyzed(report, input.actorId, updatedRun);
  ...
  private extractClaims(artifacts: StoredObject[]): ExtractionResult {
    ...
    for (const artifact of artifacts) {
      const inline = asObject(artifact.payload.inline_content);
      const rawClaims = objectArray(inline.claims);
      ...
      for (let index = 0; index < rawClaims.length; index += 1) {
        ...
        if (raw.tolerance !== undefined && numericTolerance(raw.tolerance) === null) {
        ...
        const explicitRef = typeof raw.id === "string" && raw.id.trim() ? raw.id.trim() : `${artifact.id}#claim-${index}`;
        claims.push({
          ref: explicitRef,
          artifactRef: artifact.id,
          creatorId: String(artifact.payload.created_by ?? ""),
          provenanceOrigin: String(asObject(artifact.payload.provenance).origin ?? ""),
          ...
          tolerance: numericTolerance(raw.tolerance)
        });
      }
    }
  ...
  private compare(claims: NormalizedClaim[], artifactCount: number, confidenceGapThreshold: number) {
    ...
    for (let leftIndex = 0; leftIndex < claims.length; leftIndex += 1) {
      ...
      for (let rightIndex = leftIndex + 1; rightIndex < claims.length; rightIndex += 1) {
        ...
        if (oppositeStance(left.stance, right.stance) && (!left.hasValue || !right.hasValue || left.valueKey === right.valueKey)) {
          ...
          const tolerance = Math.max(left.tolerance ?? Number.POSITIVE_INFINITY, right.tolerance ?? Number.POSITIVE_INFINITY);
          if (Number.isFinite(tolerance) && Math.abs(left.value - right.value) > tolerance) {
    ...
    const status: DisagreementStatus = hardFindings.length > 0
      ...
      : confidenceFindings.length > 0
        ...
        : artifactCount < 2 || comparablePairCount === 0
          ...
          : "compatible";
    const requiresVerification = hardFindings.length > 0;
    ...
    const compatibleSubjects = [...comparableSubjects]
      .filter((subject) => !conflictedSubjects.has(subject))
      .sort();

    return { findings, status, requiresVerification, recommendedAction, comparablePairCount, compatibleSubjects };
  }
  ...
  private finding(
    ...
    const basis = canonicalJson({ kind, subject: normalizeSubject(subject), artifactRefs, claimRefs, values, rationaleCode });
  ...
  private analysisDigest(run: StoredObject, artifacts: StoredObject[], claims: NormalizedClaim[], threshold: number): string {
    const material = {
      ...
      })).sort((a, b) => `${a.artifact_ref}:${a.ref}`.localeCompare(`${b.artifact_ref}:${b.ref}`))
    };
    return createHash("sha256").update(canonicalJson(material)).digest("hex");
  }

  private emitAnalyzed(report: StoredObject, actorId: string, run: StoredObject): void {
  ...
  private result(run: StoredObject, report: StoredObject): DisagreementAnalysisResult {
    ...
    return {
      ...
      findings: Array.isArray(inline.findings) ? inline.findings as unknown as DisagreementFinding[] : []
    };
  }
  ...
  private assertActor(run: StoredObject, actorId: string): void {
    ...
    if (!actorId.startsWith("operator_")) throw new Error(`Only Team Run leader ${leaderId} or an operator can analyze disagreement for ${run.id}`);
  }
