...
import { TeamRunFanout } from "../src/team-run-fanout.js";
...
function setup(runtime: RuntimeAdapter, options: { dbPath?: string; maxParallelWorkers?: number; createLeader?: boolean } = {}) {
  ...
  const fanout = new TeamRunFanout(teams, gateway, queue, runner);
  const supervisor = new ExecutionSupervisor(gateway, queue, runner, 0, undefined, fanout);
  return { store, queue, gateway, teams, runner, supervisor, fanout };
}
...
class BarrierRuntime implements RuntimeAdapter {
  ...
  async execute(context: RuntimeExecutionContext): Promise<RuntimeExecutionResult> {
    ...
    return {
      ...
      output: { objective: context.task.payload.objective, worker_id: (context.principal ?? context.bot).id },
...
test("parallel fan-out runs three independent Workers concurrently and collects all Artifacts", async () => {
  ...
  try {
    ...
    for (const task of snapshot.tasks) {
      assert.equal(task.payload.root_owner_id, "bot_fanout_leader");
      assert.ok(typeof task.payload.environment_lease_id === "string");
      const environmentLease = env.store.getObject(String(task.payload.environment_lease_id));
      assert.equal(environmentLease?.kind, "environment_lease");
      assert.equal(environmentLease?.payload.issued_to, task.payload.assignee_id);
      assert.equal(environmentLease?.payload.task_id, task.id);
    }
  ...
  async execute(context: RuntimeExecutionContext): Promise<RuntimeExecutionResult> {
    ...
    if (this.fastMarkers.some((marker) => objective.includes(marker))) {
      return {
        ...
        output: { objective, worker_id: (context.principal ?? context.bot).id },
        usage: { input_tokens: 0, output_tokens: 0, cost: 0, actions: 1 }
      };
    }
...
test("two stale retry-safe fan-out Workers recover after database reopen and satisfy the persisted join", async () => {
...

  {
    ...
    try {
      ...
      const snapshot = fanout.snapshot(runId, fanoutId);
      assert.equal(snapshot.status, "satisfied");
      assert.equal(snapshot.artifacts.length, 2);
...
test("parallel fan-out fails closed when Worker creation permission is absent", () => {
  const store = new CoordinationStore(":memory:");
  const queue = new ExecutionQueue(store.dbPath);
  const policy = new CoordinationPolicy(store, { requireRegisteredBots: true });
  const gateway = new CoordinationGateway(store, queue, policy);
  const manifest = leader("deterministic", 2);
  gateway.createBot({ ...manifest, permissions: { ...manifest.permissions, can_create_workers: undefined } as any });
  const teams = new TeamRunCoordinator(store, gateway, policy);
  const runner = new BotRunner(store, gateway, queue, new RuntimeRegistry().register(new DeterministicRuntimeAdapter()));
  const fanout = new TeamRunFanout(teams, gateway, queue, runner);
  try {
    const run = createRun(teams, { max_workers: 2, max_actions: 2 });
    assert.throws(() => fanout.createFanout({
      runId: run.id,
      createdBy: "bot_fanout_leader",
      workers: [worker(901)]
    }), /not explicitly allowed/);
    assert.equal(teams.listWorkers(run.id).length, 0);
  } finally {
    queue.close();
    store.close();
  }
